...
for i=1:length(paramsError)
    ...
    estimatedModelParameters.parametersError = paramsError{i};
    R_estimator_params.Kp1 = kp1*(1+paramsError{i}/100);
    R_estimator_params.qmax = qp_max*(1+paramsError{i}/100);
    R_estimator_params.kips = Kips*(1+paramsError{i}/100);
    R_estimator_params.kps = Kps*(1+paramsError{i}/100);
...
if (savePlots==1)
    saveas(fig, '../Informes/Images_tp2/kalman', 'png');
end
...
legend(sprintf('sin error (MSE=%.3f)', MSE{1}), sprintf('error al 5% (MSE=%.3f)', MSE{2}), sprintf('error al 10% (MSE=%.3f)', MSE{3}), sprintf('error al 40% (MSE=%.3f)', MSE{4}));

if (savePlots==1)
    saveas(fig, '../Informes/Images_tp2/kalman_error', 'png');
end